-- =====================================================
-- COMPLETE SUPABASE SETUP FOR BIRTHDAY CARD APP
...
-- 11. CREATE FUNCTIONS FOR CARD MANAGEMENT
...
-- 12. CREATE STATISTICS FOR BETTER PERFORMANCE
CREATE STATISTICS birthday_cards_stats ON birthday_cards (username, is_public, created_at);
...
-- These queries should return results to verify setup is working

-- Verify table exists
SELECT 'birthday_cards table exists' as status 
WHERE EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'birthday_cards');

-- Verify bucket exists
SELECT 'birthday-cards bucket exists' as status
FROM storage.buckets 
WHERE id = 'birthday-cards';

-- Verify sample data
SELECT 'Sample cards created' as status, COUNT(*) as count
FROM birthday_cards 
WHERE username LIKE 'sample-%';

-- Verify RLS is enabled
SELECT 'RLS enabled' as status, rowlevelsecurity 
FROM pg_tables 
